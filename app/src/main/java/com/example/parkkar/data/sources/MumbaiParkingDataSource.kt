...
class MumbaiParkingDataSource(private val context: Context) : ParkingDataSource {

    private val json = Json { ignoreUnknownKeys = true }

    override suspend fun getParkingLocations(): List<ParkingLocation> {
        return try {
            val jsonString = context.assets.open("mumbai_parking.json").bufferedReader().use { it.readText() }
            val mumbaiRoot = json.decodeFromString<MumbaiRoot>(jsonString)
            mumbaiRoot.toParkingLocationList()
        } catch (e: IOException) {
            e.printStackTrace()
            emptyList()
